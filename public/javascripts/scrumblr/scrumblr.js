...
function getMessage(m) {
    ...
    switch (action) {
        ...
        default:
            //unknown message
            console.log('scrumblr says: unknown action: ' + JSON.stringify(message));
            break;
    }
}
...
function drawNewCard(id, text, x, y, rot, colour, sticker, animationspeed) {
    ...
    console.log("...drawNewCard: "+id);
    var h = '<div id="' + id + '" class="scrumblrcard ' + colour +
        ' draggable" style="-webkit-transform:rotate(' + rot +
        'deg);\
	">\
	<img src="/images/scrumblr/icons/token/Xion.png" class="scrumblrcard-icon delete-scrumblrcard-icon" />\
	<img class="scrumblrcard-image" src="/images/scrumblr/' +
        colour + '-card.png">\
	<div id="content:' + id +
        '" class="scrumblrcontent stickertarget droppable">' +
    ...
    card.hover(
        function() {
            $(this).addClass('hover');
            $(this).children('.scrumblrcard-icon').fadeIn(10);
        },
        function() {
            $(this).removeClass('hover');
            $(this).children('.scrumblrcard-icon').fadeOut(150);
        }
    );

    card.children('.scrumblrcard-icon').hover(
        function() {
            $(this).addClass('scrumblrcard-icon-hover');
        },
        function() {
            $(this).removeClass('scrumblrcard-icon-hover');
        }
    );

    card.children('.delete-scrumblrcard-icon').click(
        function() {
            $("#" + id).remove();
    ...
    card.children('.scrumblrcontent').editable(function(value, settings) {
        ...
        submit: 'OK',
        style: 'inherit',
        cssclass: 'scrumblrcard-edit-form',
...
function addSticker(cardId, stickerId) {
    ...
    if (Array.isArray(stickerId)) {
        for (var i in stickerId) {
            stickerContainer.prepend('<img src="/images/scrumblr/stickers/' + stickerId[i] +
                '.png">');
        }
    } else {
        if (stickerContainer.html().indexOf(stickerId) < 0)
            stickerContainer.prepend('<img src="/images/scrumblr/stickers/' + stickerId +
                '.png">');
    }
}
...
function changeThemeTo(theme) {
    ...
    $("link[title=cardsize]").attr("href", "/stylesheets/scrumblr/" + theme + ".css");
}
...
function boardResizeHappened(event, ui) {
    var newsize = ui.size;
    console.log("boardResizeHappend: "+newsize);
    sendAction('setBoardSize', newsize);
}

function resizeBoard(size) {
  console.log("resizeBoard: "+size)
...
$(function() {
    ...
    $("#smallify").click(function() {
        ...
        console.log("smallify after: "+currentTheme);
